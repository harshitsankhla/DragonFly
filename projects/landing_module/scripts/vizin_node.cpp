...
geometry_msgs::PoseStamped viz_cb(const geometry_msgs::PoseStamped::ConstPtr& msg)
{
	goal_position = *msg;
	pose.pose.position.x = goal_position.pose.position.x;
	pose.pose.position.y = goal_position.pose.position.y;
	return pose;
}
...
int main(int argc, char **argv)
{
	...
	ros::Subscriber viz_loc =nh.subscribe<geometry_msgs::PoseStamped>("move_base_simple/goal", 5, viz_cb);
