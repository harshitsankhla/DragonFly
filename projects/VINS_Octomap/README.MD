...
In this project, we focus on creating a good map that will be later used for navigating our quadcopter. Our aim is to make an [Octomap](https://octomap.github.io/) which is a 3D occupancy map using Visual-Inertial Odometry and, optionally, global fusion with GPS. The map created can be later used for navigating drones in the environment, by way-point extraction through path planning algorithms. 
